...
const FORM: FormBuilderFormType = {
    ...
    sections: [
        ...
        },
        {
            ...
            labelClassName: "w-full max-w-[250px] min-w-[125px] text-[13px] leading-[32px] font-medium",
            fields: [
                {
                    label: "Blood Group",
                    name: "bloodGroup",
...
const MemberForm: React.FC<MemberFormPropsType> = ({ className, children, ...rest }: MemberFormPropsType) => {
    const classes = useStyles();
    const initialValues = createFormikInitialValue(FORM);
    const validationSchema = createFormikYupSchema(FORM);

    const onSubmitHandler = (values: MemberFormType, { setSubmitting }: FormikHelpers<MemberFormType>): void => {
        setSubmitting(true);
    ...
    return (
        <Formik initialValues={initialValues as MemberFormType} validationSchema={validationSchema} onSubmit={onSubmitHandler} {...rest}>
