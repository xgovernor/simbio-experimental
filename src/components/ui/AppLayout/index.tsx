...
const AppLayout = ({
  ...
  const store = useSelector((state: TStoreState) => state.layout);
  ...
  return layout ? (
    ...
    >
      ...
      <Canvas className="col-span-1 col-start-2 col-end-3 row-span-1 row-start-2 row-end-3 block h-[calc(100vh-92px)] w-full overflow-y-auto md:relative">
        {store.title && <CanvasHeader title={store.title} icon={store.icon} />}
        {store.breadcrumb && <CanvasBreadcrumb links={store.breadcrumb} />}
        {/* <Transition> */}
        {children}
        {/* </Transition> */}
      </Canvas>
