"use client";
import Image from "next/image";
import Link from "next/link";
import { memo } from "react";
import Layout from "@/components/ui/Layout";
import WELCOME_ILLUSTRATION from "@/assets/images/welcome-Illustration.png";
import { Button } from "@/components/ui/button";

const MENU: { id: string; title: string; url: string }[] = [
  {
    id: "1",
    title: "About",
    url: "/about",
  },
  {
    id: "2",
    title: "Privacy",
    url: "/privacy-policy",
  },
  {
    id: "3",
    title: "Terms",
    url: "/terms",
  },
  {
    id: "4",
    title: "Donation",
    url: "/donation",
  },
];

const PageWelcome: React.FC = () => {
  return (
    <Layout>
      <div
        className="mx-auto w-full max-w-[536px] flex h-full flex-col flex-nowrap items-center justify-center text-center p-5"
        style={{
          minHeight: "calc(100vh - 92px)",
        }}
      >
        <Image
          className="mb-8"
          src={WELCOME_ILLUSTRATION}
          alt="SIMBIO - World first & largest online blood network"
          loading="lazy"
          blurDataURL="data:image/svg+xml;base64,"
        />
        <h2 className="text-xl font-bold">Welcome to SIOMBIO. We are glad to have you!</h2>

        <p className="text-sm mt-2">SIMBIO is your tool to find, manage and organize blood donations.</p>

        <Button className="mt-8 w-full max-w-[280px]" variant="default" asChild>
          <Link  href="#">
            Update your profile
          </Link>
        </Button>
      </div>
    </Layout>
  );
};

export default memo(PageWelcome);
